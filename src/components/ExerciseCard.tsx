...
const visualGlyph: Record<string, string> = { warmup: '↻', cardio: '→', strength: '↑', balance: '◇', cooldown: '↓', recovery: '∿' };
const visualAsset: Record<string, string> = { 'chair-squat': '/movement/chair-squat.png', 'wall-pushup': '/movement/wall-pushup.png', walk: '/movement/walking.png', march: '/movement/walking.png' };
...
export function ExerciseCard({ target, index, total, consent, onPrevious, onComplete, onSkip }: { target: ExerciseTarget; index: number; total: number; consent: boolean; onPrevious: () => void; onComplete: () => void; onSkip: () => void }) {
  ...
  return <div className="session-page">
    ...
    {visualAsset[item.id] ? <img className="movement-image" src={visualAsset[item.id]} alt={`Contoh ${item.title}`} /> : <div className={`movement-visual ${item.kind}`} role="img" aria-label={`Ilustrasi konseptual ${item.title}`}><span>{visualGlyph[item.kind]}</span><small>{item.kind.toUpperCase()}</small></div>}
