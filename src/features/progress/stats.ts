...
import type { ExerciseKind, FreeSessionTemplate, JournalEntry } from '../../domain/types';
...
export function buildProgressStats(sessions: SessionLog[], journals: JournalEntry[], templates: FreeSessionTemplate[], today: string): ProgressStats {
  ...
  completed.flatMap((session) => session.completedItemIds).forEach((id) => { const exercise = MODE1_EXERCISES[id]; if (exercise) categoryCounts[exercise.kind] += 1; });
  ...
  return { completedCount: completed.length, totalMinutes, sessionSources, categoryCounts, calendar, journalAverages, achievements, summary: `${completed.length} sesi selesai dan ${totalMinutes} menit bergerak.` };
}
